...
describe("Rover class", function() {
  ...
  it("response returned by receiveMessage contains the name of the message", function() {
    let newMessage = new Message("name of message", [new Command("command1", "command1 type"), new Command("command2", "command2 type"), new Command("command3", "command3 type")])
    expect(new Rover().recieveMessage(newMessage).message).toBe('name of message')
  })

  it("response returned by receiveMessage includes two results if two commands are sent in the message", function() {
    let newMessage = new Message("name of message", [new Command("command1", "command1 type"), new Command("command2", "command2 type")])
  ...
  it("responds correctly to the status check command", function() {
    ...
    let rover = new Rover()
    expect(rover.recieveMessage(newMessage).results.RoverStatus).toBe({roverStatus: {mode: rover.mode, generatorWatts: rover.generatorWatts, position: rover.position}})
  })

  it("responds correctly to the mode change command", function() {
    let newMessage = new Message("mode change", [new Command("MODE_CHANGE", "LOW_POWER")])
    let rover = new Rover().recieveMessage(newMessage)
  ...
  it("responds with a false completed value when attempting to move in LOW_POWER mode", function() {
    ...
    let rover = new Rover(50).recieveMessage(newMessage)
  ...
  it("responds with the position for the move command", function() {
    ...
    let rover = new Rover(50).recieveMessage(moveMessage)
